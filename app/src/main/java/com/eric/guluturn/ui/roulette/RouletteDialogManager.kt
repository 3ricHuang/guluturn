 ...
 * including the result card view, rejection reason input, and session-end summary.
 *
 ...
 * @param onPlayAgain callback invoked when user chooses to restart session
 */
class RouletteDialogManager(
    ...
    /**
     * Shows the session-end dialog with a message, QR code, and a "Play Again" button.
     *
     * @param acceptedRestaurant the accepted restaurant, or null if all were rejected
     */
    fun showSessionEndDialog(acceptedRestaurant: AcceptedRestaurant?) {
        val view = LayoutInflater.from(context).inflate(R.layout.dialog_session_complete, parent, false)
        val dialog = AlertDialog.Builder(context, R.style.DialogTheme)
            .setView(view)
            .setCancelable(false)
            .create()

        val messageView = view.findViewById<TextView>(R.id.tvSessionMessage)
        val qrImage = view.findViewById<ImageView>(R.id.qrCodeImage)
        val playAgainButton = view.findViewById<Button>(R.id.btnPlayAgain)

        messageView.text = if (acceptedRestaurant != null) {
            context.getString(R.string.session_message_accept, acceptedRestaurant.name)
        } else {
            context.getString(R.string.session_message_reject)
        }

        // Show QR code (already statically set via @drawable/qrcode)
